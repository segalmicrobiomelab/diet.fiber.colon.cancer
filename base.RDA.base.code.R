...
LPL <- read_csv("msq_fiber_matadate_cells.csv")
Genus_II <- read_csv("genustable_122723.csv")
...
rownames(LPL) <- LPL$SampleID
rownames(Genus_II) <- Genus_II$SampleID
...
Genus_II_hellinger <- decostand(Genus_II, method = "hellinger")

# Predicting transformed species using all variables contained in LPL
my_rda <- rda(Genus_II_hellinger ~ ., data = LPL)
...
pdf(file="Genus_RDA_plot.pdf", width=8, height=8)
...
ggsave("Genus_RDA_plot.pdf", height=8, width=10, device="pdf")
dev.off()
